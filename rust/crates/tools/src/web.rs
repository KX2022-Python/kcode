...
use crate::types::{
    WebBrowserInput, WebBrowserOutput, WebFetchInput, WebFetchOutput, WebSearchInput,
    WebSearchOutput,
};
...
pub(crate) fn execute_web_fetch(input: &WebFetchInput) -> Result<WebFetchOutput, String> {
    ...
    Ok(WebFetchOutput {
        ...
        result: summarize_web_fetch(&final_url, &input.prompt, &normalized, &body, &content_type),
...
pub(crate) fn execute_web_search(input: &WebSearchInput) -> Result<WebSearchOutput, String> {
    ...
    let client = build_http_client()?;
    let response = client
        .get(build_search_url(&input.query)?)
        .send()
        .map_err(|error| error.to_string())?;
...
pub(crate) fn execute_web_browser(input: WebBrowserInput) -> Result<WebBrowserOutput, String> {
    let message = match input.action.as_str() {
        "navigate" => {
            let url = input.url.as_deref().ok_or("navigate requires a url")?;
            return Ok(WebBrowserOutput {
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("kcode-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn extract_search_hits(html: &str) -> Vec<SearchHit> {
    ...
    while let Some(anchor_start) = remaining.find("result__a") {
        ...
        let Some((url, rest)) = extract_quoted_value(&after_class[href_idx + 5..]) else {
            remaining = &after_class[1..];
            continue;
        };
        ...
        if let Some(decoded_url) = decode_duckduckgo_redirect(&url) {
            hits.push(SearchHit {
                title: html_to_text(&after_tag[..end_anchor_idx])
                    .trim()
                    .to_string(),
                url: decoded_url,
            });
...
fn extract_search_hits_from_generic_links(html: &str) -> Vec<SearchHit> {
    ...
    while let Some(anchor_start) = remaining.find("<a") {
        ...
        let Some((url, rest)) = extract_quoted_value(&after_anchor[href_idx + 5..]) else {
            remaining = &after_anchor[2..];
            continue;
        };
        let Some(close_tag_idx) = rest.find('>') else {
